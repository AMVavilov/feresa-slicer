...
| Status | User function | Evidence and boundary |
| --- | --- | --- |
| **WORKING** | Load connection details from an Orca printer profile | `print_host`, host type, port, API key and username/password are parsed from the selected profile (`../app/src/main/java/tech/g24/feresaslicer/auth/OrcaPrinterConnection.kt:29-51`) and displayed without revealing secrets (`../app/src/main/java/tech/g24/feresaslicer/ui/FeresaSlicerApp.kt:1703-1735`). |
| **WORKING** | Moonraker/Klipper and OctoPrint test/upload/start path | The client tests the expected JSON endpoints and uploads/starts G-code through each protocol (`../app/src/main/java/tech/g24/feresaslicer/printer/NetworkPrinterClient.kt:23-97`). API-key and HTTP Basic headers are both applied (`:160-171`); header/auth status behavior has JVM tests (`../app/src/test/java/tech/g24/feresaslicer/printer/PrinterRequestHeadersTest.kt:11-51`, `../app/src/test/java/tech/g24/feresaslicer/auth/OrcaPrinterConnectionAuthenticationTest.kt:8-30`). |
| **WORKING** | Explicit print confirmation | Sending only occurs from a confirmation dialog that names the printer/host and warns that printing starts immediately (`../app/src/main/java/tech/g24/feresaslicer/ui/FeresaSlicerApp.kt:1322-1390`). |
| **PARTIAL** | Verification level | Request construction is unit-tested, but this audit did not upload to a physical Moonraker/OctoPrint instance. A live-printer integration test remains required before release certification. |
| **PARTIAL** | Printer management | There is no manual connection editor, LAN discovery, printer status/job/temperature controls or camera. The screen selects connection data from profiles and tests it. |
| **MISSING** | Other desktop hosts/device plugins | PrusaLink is explicitly rejected and unknown protocols cannot send (`../app/src/main/java/tech/g24/feresaslicer/printer/NetworkPrinterClient.kt:23-38`, `:40-52`). Proprietary Bambu device integration is not included. |
...
| **WORKING** | Secure persisted session | Only the refresh token is persisted using AES-256-GCM with Android Keystore (`../app/src/main/java/tech/g24/feresaslicer/auth/EncryptedRefreshTokenStore.kt:14-74`); session restore/refresh and sign-out are implemented (`../app/src/main/java/tech/g24/feresaslicer/auth/OrcaAuthViewModel.kt:86-159`). |
...
| **PARTIAL** | Cached profile confidentiality | The refresh token is encrypted, but full pulled profile JSON is a plaintext file in app-private storage (`../app/src/main/java/tech/g24/feresaslicer/auth/OrcaProfileCache.kt:15-58`). Printer profiles can contain connection secrets; filesystem encryption/backup policy must remain part of the threat model. |
